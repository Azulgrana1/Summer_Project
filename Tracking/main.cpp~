...
Scalar BLUE_MIN(100, 0, 0);
Scalar BLUE_MAX(255, 100, 50);
...
const string windowName3 = "Blue Thresholded Image";
...
{   
    ...
    bool enable_following = 0;
    char status[1];
    char cmd;

    while(1){
        cmd = waitKey(10); //delay 10ms  and get key
        ...
        inRange(cameraFeed,BLUE_MIN,BLUE_MAX,blue_threshold);
        ...
        if(trackObjects)
        {
            ...
            blue_on = trackFilteredObject(blue_x, blue_y, blue_threshold,cameraFeed);
        }
...
/*
        ...
        //switch modes
        if(mode[0] == 'k' && cmd == 'w') //key 'w' pressed during auto mode
        {
            mode[0] = 'w'; //switch to follow mode
            write(fd, mode, 1);
        }
        else if (mode[0] == 'w' && cmd == 'k')//key pressed during follow mode
        {
            mode[0] = 'k';
            write(fd, mode, 1);
        }
        ...
        if(mode[0] == 'k' )
        {
            ...
            if(blue_on) //Object found
            {
                 if(blue_x < cameraFeed.cols/3)
                    status[0] = 'j';
                else if(blue_x > 2*cameraFeed.cols/3)
                    status[0] = 'l';
                else
                    status[0] = 'i';
            }
        ...
        imshow(windowName3, blue_threshold);
